...
| `bundle` | Runs `make bundle` in every image-bearing member — `APP_DIRS` apps + vllm-service + data-plane (active profile) + open-webui-service (`OPENWEBUI_DIR`) + obs-plane (`OBS_DIR`) + edge-plane (`EDGE_DIR`) — then saves the `wait-healthy.sh` probe image (digest-pinned via `WAIT_PROBE_PIN`) as `wait-probe-image.tar.gz` in this repo. A member whose bundle for the current release already exists — clean tree, `.<slug>-version` recording the latest reachable tag, tarball present — is skipped with a `>> <member>: bundle <ver> already present — skipping` line; `BUNDLE_FORCE=1` forces the full fan-out. Every image-bearing member writes that record from the vendored `scripts/bundle-lib.sh` (`bundle_version` → `.<slug>-version`, gitignored), so the skip applies to all of them; a `*VERSION_OVERRIDE` in the environment disables it for that run. |
